...
/*
File name: vs.php
Last change: Sat Jan 12 13:22:47 EET 2008
...
if ($filename == "")
{
   ...
   //$files = preg_find('/\.php$/D', "/", PREG_FIND_RECURSIVE);
   head("Files view ({$context})");
   prnt("<b>Usage 1: admin.php?module={$module}&file=index.php</b>", true);
   prnt("<b>Usage 2: admin.php?module={$module}&context=./admin/</b><br>", true);
   $files = preg_find('/\.php$/D', $context, PREG_FIND_SORTBASENAME);
   foreach ($files as $value) prnt(create_link("admin.php?module={$module}&file={$value}", $value), true);
}
else
{
   head("Code view ({$filename})");
